...
def player_turn():
    global enemy_health
    action = input("어떤 행동을 하시겠습니까? (1: 건초어택/ 2: 사료스킬)")
    if action == '1':
...
def attack(character_name, attach_damage):
    print(f'"{character_name}"의 공격! 적에게 {attach_damage}의 피해를 입혔습니다."')
...
if player_health > 0:
    print(f'"{player_name}"님이 승리하였습니다.')
else:
    print(f'"{player_name}"님이 패배하였습니다ㅠㅠ')
